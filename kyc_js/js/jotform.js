    ...
    */
    ...
    handleIFrameHeight: function () {
        ...
        if (isEmbedForm) {
          ...
          try{
            if (window.grecaptcha && typeof window.grecaptcha.reset === 'function' && !window.grecaptcha.resetted) {
  ...
  handleSignSignatureInputs: function() {
    ...
    validateAll: function (form, scopeSelector) {
        function handleValidateAll(form, scopeSelector) {
            ...
            if (JotForm.payment === 'square' && typeof window.ValidatePaymentGateways !== 'undefined' && !!document.querySelector('.form-pagebreak') && JotForm.isVisible($$('[data-payment="true"]')[0])) {
                ret = window.ValidatePaymentGateways.validateSquare(true)
            }
